...
| 📦 最新版本 | v10.5.1 |
| 🏭 行业覆盖 | **32个申万一级行业**（四大类别） |
| 📊 案例规模 | **32个案例**（14个深度分析 + 18个模板框架） |
| 🤖 自动化 | GitHub Actions 全链路 CI/CD，每日 UTC 03:00 自动更新 |
| 🔌 数据源 | akshare（A股财务，东方财富/同花顺）— 免费 |
| ⏱️ 数据时效 | 每个案例明确标注**报告期**和**拉取日期**，过期数据会标注 ⚠️ |
| 📄 许可证 | MIT |

> ⚠️ **重要声明**：所有财务数据来自公开数据源（akshare），报告期标注在每个案例的自动洞见区块中。**评分和结论基于对应报告期的数据，数据过期后结论可能失效。** 每日 CI 流水线自动更新数据。
...
| 数据 | 更新频率 | 来源 | 何时过时 |
|------|:-------:|------|:--------:|
| **毛利率/ROE** | 财报发布后更新 | akshare 抓取上市公司财报 | 下一期财报发布后 |
| **市场规模** | 随代表股营收更新 | 估算（代表股营收×倍数） | 季度变化 |
| **评分/洞见** | 数据更新时自动重算 | 规则引擎（基于最新指标） | 指标变化时 |
| **行业分类** | 申万指数调整时 | 申万一级（2024版） | 下次行业分类调整 |

### 如何判断数据是否过期

1. 查看案例中自动洞见的 **财务报告期**——如果报告期距今超过 1 年，数据已严重过时
2. 查看时间序列趋势——**毛利率趋势**和 **ROE 趋势**会显示最近 3-4 期的年化变化方向
3. 仪表盘页顶的 **数据时效** 横幅会显示全行业最新报告期
